...
const emptyFeed: FeedState = { tracks: [], nextCursor: null, city: null };
...
export default function LocalArtistsFeed() {
  ...
  const [isMuted, setIsMuted] = useState(true); // Браузер дозволяє autoplay тільки в muted-режимі.
  ...
  // Зупиняємо глобальний міні-плеєр, поки користувач у фіді.
  useEffect(() => {
    pauseGlobalAudio();
  }, [pauseGlobalAudio]);
  ...
  const loadInitialFeed = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await getFeed({ limit: PAGE_SIZE });
      setFeed({
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const response = await getFeed({ limit: PAGE_SIZE, cursor: feed.nextCursor });
      setFeed((prev) => ({
  ...
  // На першому вході перевіряємо, чи в користувача вже задано місто; якщо ні — пропонуємо геолокацію.
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
  ...
  // Спостерігаємо за тим, який слайд зараз у в'юпорті.
  useEffect(() => {
  ...
  // Підвантажуємо наступну сторінку, коли активний слайд у нижній третині списку.
  useEffect(() => {
  ...
  // Перемикаємо аудіо джерело й автоплей при зміні активного треку.
  useEffect(() => {
    ...
    audio
      ...
      .catch((err: unknown) => {
        // AbortError зустрічається при швидкій зміні слайдів — не помилка.
        ...
        >
          {feed.tracks.map((track, index) => (
            <FeedSlide
              ...
              onToggleLike={() => handleLike(track.id)}
